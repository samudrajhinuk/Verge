...
On a normal real-estate site the photographs are the presentation and video, if it exists at all, is a supplement — an extra tab, a YouTube embed below the gallery, something sized to whatever space the layout had left over. On Verge video **is** the presentation. The card is not a photo with a play button on it; the card is the clip. The grid is built around the 9:16 ratio rather than cropping footage to fit a shape chosen first: the column count at each breakpoint is derived from that ratio (§6), which is why there are four columns at 1440px and not three. The layout follows the footage; the footage is not squeezed into the layout. That is the whole claim — a presentational and structural difference, not a feature list.

> *Note — the shot list, designed and cut.* An earlier version of this document specified a **shot list**: a timestamped index beside each video ("0:11 Living room"), every line a button seeking the clip to that moment, giving video's continuity alongside a gallery's random access. It was fully specified and it was cut when the real footage arrived showing one room per clip rather than a walkthrough — there is nothing to index into, so every timestamp would have been invented. Recorded here as a scoping decision made on contact with the actual material, not a failure: the feature was sound, the content did not support it honestly. `videoCaption` is what remains of it. See DECISIONS.md, "Video model change".
